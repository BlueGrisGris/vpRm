...
proc_drivers.vpRm <- function(vpRm){
	...
	time(temp) <- parse_herbie_hrrr_times(vpRm$dirs$temp_dir)
	...
	time(par) <- parse_herbie_hrrr_times(vpRm$dirs$par_dir)
	if(vpRm$verbose){Print_Info(temp)}
	...
	time(evi) <- parse_modis_evi_times(vpRm$dirs$evi_dir)
	if(vpRm$verbose){Print_Info(temp)}
